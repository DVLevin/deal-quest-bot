...
**f. Add handleRegenerate with undo:**
```typescript
const handleRegenerate = useCallback(() => {
  onGenerateDraft();
  // After regeneration completes, parent will set previousDraftResult
  // and the undo toast will appear
}, [onGenerateDraft]);
```
...
Verify StepActionScreen accepts `draftResult` prop and passes it to DraftCopyCard.
...
11. Test regeneration: open another step with screenshot, generate draft, then tap the refresh icon — verify new draft replaces old one
12. Check Langfuse dashboard — verify a trace for `agent:comment_generator` appears with the generation
13. (Optional) Set a model override for `comment_generator` in admin panel and verify it's used on next generation
  </how-to-verify>
