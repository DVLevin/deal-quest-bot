...
<objective>
Integrate the StepActionScreen into LeadDetail, wire up all hooks (upload, step update with proof/can't-perform), and upgrade deep link behavior so ?step=X opens the step in action mode. Add proof indicators to LeadCard.
...
<task type="auto">
  <name>Task 1: Wire StepActionScreen into LeadDetail with deep link auto-expand</name>
  <files>
    packages/webapp/src/features/leads/components/LeadDetail.tsx
  </files>
  <action>
Modify LeadDetail.tsx to replace the compact step rows with an expandable action screen pattern.
...
5. Run `cd packages/webapp && npx vite build` -- build succeeds
  </verify>
  <done>
LeadDetail renders StepActionScreen when a step is active. Deep links auto-expand steps into action mode. Compact rows show proof indicators and are tappable. All mutation callbacks wired correctly. Build succeeds.
  </done>
</task>
...
10. Test deep link: send a bot reminder with "Open in App" button -- tapping should open the specific step in action mode
  </how-to-verify>
