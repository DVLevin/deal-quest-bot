...
must_haves:
  truths:
    ...
    - "User sees API key status and can deep-link to bot for key changes"
  artifacts:
...
- Add `<ActivityTimeline leadId={leadId} />` as the last section with a "Activity" header and `History` icon from lucide-react
