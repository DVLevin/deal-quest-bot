...
<task type="auto">
  ...
  <action>
    ...
    4. Create `ABBranching.tsx`:
       ...
       - This is a reusable pattern -- currently wired with placeholder options since branching scenarios don't exist yet in the data (per research Open Question 3)
    ...
    5. Update `Train.tsx`:
       - Replace stub content with nested Routes (React Router v7):
         - `index` -> Difficulty filter screen (DifficultyFilter + "Start Training" button)
         - `scenario` -> Scenario display (ScenarioCard + TimerInput)
       - State management with useState: `selectedDifficulty`, `currentScenario`
       - On "Start Training": pick a random scenario from the filtered pool (using useScenarioPool), navigate to /train/scenario
       - Pass scenario data via React state (useLocation state or a simple Zustand store if needed -- prefer useState lifted in Train.tsx since it's one level)
       - Actually, since Routes are nested, keep the Train.tsx as a layout with state, and render sub-views conditionally or via routes
       - Alternative simpler approach: Use a step-based state machine in Train.tsx instead of nested routes (step: 'filter' | 'scenario'). This avoids prop-passing complexity with routes. Render DifficultyFilter when step is 'filter', render ScenarioCard + TimerInput when step is 'scenario'. Use a "Back to filters" handler that resets step.
  ...
  <done>
    Train page shows difficulty selection, renders random scenario card with persona details and countdown timer. MainButton deep links to bot for practice. ABBranching component provides reusable A/B decision pattern using native Telegram buttons. Build passes cleanly.
  </done>
</task>
...
- TRAIN-06: ABBranching component wires MainButton + SecondaryButton for A/B sales decisions (reusable pattern, ready for branching scenarios)
