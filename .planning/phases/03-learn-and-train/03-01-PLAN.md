...
<task type="auto">
  ...
  <action>
    ...
    3. Create `features/learn/hooks/useTrackProgress.ts`:
       ...
       - Handle empty track_progress gracefully (per Pitfall 5 in research). Specifically: when telegramId exists but the track_progress query returns 0 rows, map over TRACKS[trackId].levels and for each level by index: if index === 0 (first level), set status to 'unlocked'; for all other levels, set status to 'locked'. All levels get bestScore: 0 and attemptsCount: 0. This ensures new users see the first level as tappable and the rest as locked without requiring any DB seeding.
  ...
  <verify>
    ...
    Verify useTrackProgress has explicit 0-rows handling: grep for a condition that checks if progress data is empty/length===0 and applies the index===0 unlocked / rest locked logic.
  </verify>
  <done>
    Static track data contains all 4 levels from scenarios.json. Query keys extended with trackProgress and scenarios. useTrackProgress hook merges static data with DB progress and handles the empty-rows case by defaulting first level to 'unlocked' and rest to 'locked' with bestScore: 0, attemptsCount: 0.
  </done>
</task>
