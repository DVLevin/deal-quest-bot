...
<task type="auto">
  ...
  <done>
    Scoring types with safe parser handle non-deterministic LLM output. CSS @property enables zero-JS score counter animation. Three shared scoring components render score circle, criteria breakdown, and strengths/improvements with full null safety.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    1. Create `features/learn/components/LevelResults.tsx`:
       ...
       - If attempt found: parse feedback_json with `parseFeedback()`, then render:
         - ScoreDisplay with score and xp_earned
         ...
         - StrengthsList with strengths, improvements, and ideal comparison
  ...
  <done>
    LevelResults shows learn scores with pass/fail status and next-level-unlock indication. ScoreResults shows train scores with animated display and 3 quick action buttons. Both views safely parse feedback_json and handle missing attempts gracefully. Routes wired in both Learn.tsx and Train.tsx.
  </done>
</task>
...
<success_criteria>
- LEARN-04: Score and feedback display shows score/100, strengths, improvements, ideal response comparison
