...
must_haves:
  truths:
    ...
    - "User can toggle 'My Entries' to see only entries they created vs all team entries"
  ...
  artifacts:
    - path: "packages/webapp/src/features/casebook/hooks/useCasebook.ts"
      provides: "Filtered and searched casebook entries from InsForge with optional my-entries filter"
    ...
    - path: "packages/webapp/src/features/casebook/components/CasebookFilters.tsx"
      provides: "Filter chips including My Entries toggle"
      contains: "showMyOnly"
  ...
  key_links:
    ...
    - from: "packages/webapp/src/features/casebook/hooks/useCasebook.ts"
      ...
      pattern: "\\.or\\("
    - from: "packages/webapp/src/features/casebook/hooks/useCasebook.ts"
      to: "My Entries filter"
      via: ".eq('created_from_user', telegramId) when showMyOnly is true"
      pattern: "created_from_user"
...
<objective>
Build the Casebook browser feature: searchable and filterable list of saved responses with "My Entries" toggle, detail view with full analysis, and "Use as Template" flow that deep-links to bot.

Purpose: Users can browse their team's library of high-quality saved responses, search by keyword, filter by persona/scenario/industry, toggle between all entries and their own, view full details, and use entries as templates for new support sessions.
...
4. Create `features/casebook/hooks/useCasebook.ts`:
   - Export `useCasebook(options: { keyword?: string; personaType?: string; scenarioType?: string; industry?: string; showMyOnly?: boolean })` hook
   - Import useQuery, getInsforge, useAuthStore, queryKeys
   - Build query: `getInsforge().database.from('casebook').select('*')`
   - **"My Entries" filter:** When `showMyOnly` is true AND telegramId is available, apply `.eq('created_from_user', telegramId!)` to filter to only the current user's entries. When false or omitted, show all entries (team-wide casebook, Pitfall 2 from research).
   ...
   - Query key: queryKeys.casebook.list({ keyword, personaType, scenarioType, industry, showMyOnly })
   - enabled: true (no telegramId gate needed for team-wide view; showMyOnly filter is additive)
...
Run `cd /Users/dmytrolevin/Downloads/GD_playground && pnpm --filter webapp exec tsc --noEmit` to confirm TypeScript compilation passes with no errors.
  ...
  <done>
Router has casebook/* wildcard. Debounce utility is in shared/hooks. Query keys include casebook entries. useCasebook hook builds PostgREST queries with ilike search, eq filters, and optional showMyOnly filter via .eq('created_from_user', telegramId). useCasebookEntry fetches single entries. useCasebookFilterOptions provides unique filter values. TypeScript compiles cleanly.
  </done>
</task>

<task type="auto">
  <name>Task 2: Casebook page with list, search, filters, My Entries toggle, detail view, and template action</name>
  <files>
    packages/webapp/src/features/casebook/components/SearchBar.tsx
    packages/webapp/src/features/casebook/components/CasebookFilters.tsx
    packages/webapp/src/features/casebook/components/CasebookCard.tsx
...
2. Create `features/casebook/components/CasebookFilters.tsx`:
   - Props: { personaTypes: string[]; scenarioTypes: string[]; industries: string[]; selectedPersonaType: string; selectedScenarioType: string; selectedIndustry: string; onPersonaTypeChange: (v: string) => void; onScenarioTypeChange: (v: string) => void; onIndustryChange: (v: string) => void; showMyOnly: boolean; onShowMyOnlyChange: (v: boolean) => void }
   - **"My Entries" toggle chip** at the top of the filter section (before persona/scenario/industry rows):
     - A single toggle chip labeled "My Entries"
     - When active: bg-brand text-white (same as active filter chip style)
     - When inactive: bg-bg-secondary text-text-hint (same as inactive chip style)
     - Clicking toggles showMyOnly between true and false via onShowMyOnlyChange(!showMyOnly)
     - Use User icon from lucide-react next to the label for visual clarity
     - Placed in its own row or inline before the first filter row, visually separated
...
6. Replace `pages/Casebook.tsx` with nested sub-routes:
   ...
   - State management in CasebookHome:
     ...
     - **showMyOnly (boolean state, default false)** -- pass to CasebookFilters and useCasebook
     - useCasebookFilterOptions() for chip values
     - useCasebook({ keyword: debouncedSearch, personaType: selectedPersonaType, scenarioType: selectedScenarioType, industry: selectedIndustry, showMyOnly })
   - Two views:
     a. Index route (CasebookHome): SearchBar at top, CasebookFilters below (including My Entries toggle), CasebookList with results. Each card onClick navigates to `/casebook/${entry.id}`
  ...
  <verify>
Run `cd /Users/dmytrolevin/Downloads/GD_playground && pnpm --filter webapp exec tsc --noEmit` to confirm TypeScript compilation passes. Then run `cd /Users/dmytrolevin/Downloads/GD_playground && pnpm --filter webapp build` to confirm production build succeeds. Verify CasebookFilters.tsx contains showMyOnly prop and a "My Entries" toggle chip with User icon. Verify useCasebook.ts applies .eq('created_from_user', telegramId) when showMyOnly is true.
  </verify>
  <done>
Casebook page renders with two sub-routes. Index shows search bar, "My Entries" toggle chip, filter chips, and filtered casebook list. When "My Entries" is active, only entries with created_from_user matching current user's telegramId are shown. Detail view shows full entry with all sections and "Use as Template" button. Search is debounced at 300ms with special character escaping. Filter chips show actual values from the database. Empty states handle both no-results and no-entries scenarios. Copy button works with clipboard API fallback. Build passes cleanly.
  </done>
</task>
...
4. Casebook queries default to team-wide (no user filter) unless "My Entries" is active
5. When "My Entries" toggle is active, useCasebook applies .eq('created_from_user', telegramId)
6. CasebookFilters has showMyOnly prop and renders a "My Entries" toggle chip with User icon
7. Search escapes % and _ characters before sending to PostgREST ilike
8. Search input is debounced at 300ms (not firing on every keystroke)
9. Filter chips populated from actual database values via useCasebookFilterOptions
10. Detail view has Copy button with clipboard fallback and "Use as Template" deep link to bot
11. Empty states are contextual (different messages for no results vs no entries)
</verification>
...
- "My Entries" toggle chip in CasebookFilters allows user to switch between team-wide and personal entries
- Router wildcard fixed before sub-routes built
- Team-wide casebook by default, user-only when "My Entries" toggled on
