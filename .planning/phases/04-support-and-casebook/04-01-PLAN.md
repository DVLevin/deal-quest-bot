...
must_haves:
  truths:
    ...
    - "User sees a dedicated 'Send Screenshot' button that deep-links to bot for photo-based analysis"
  ...
  key_links:
    ...
    - from: "packages/webapp/src/features/support/components/SupportInput.tsx"
      to: "Bot deep link (text analysis)"
      via: "openTelegramLink for Start Analysis CTA"
      pattern: "openTelegramLink"
    - from: "packages/webapp/src/features/support/components/SupportInput.tsx"
      to: "Bot deep link (screenshot analysis)"
      via: "openTelegramLink for Send Screenshot button"
      pattern: "openTelegramLink.*support_photo|Camera"
...
<objective>
Build the Support mode feature: strategy builder input CTA (deep link to bot), screenshot upload affordance (deep link to bot for photo analysis), structured analysis display for past sessions, engagement tactics, draft response with copy/regenerate/save actions, and session history browsing.

Purpose: Users can get AI-powered deal support by starting sessions in the bot, then view rich structured analysis results in the TMA with prospect type, signal strength, closing strategy, engagement tactics, and copyable draft responses. Screenshot analysis is explicitly surfaced as a capability via a dedicated button.
...
<task type="auto">
  <name>Task 2: Support page with display components, screenshot button, and sub-routes</name>
  <files>
    packages/webapp/src/features/support/components/SupportInput.tsx
    packages/webapp/src/features/support/components/AnalysisDisplay.tsx
    packages/webapp/src/features/support/components/StrategyDisplay.tsx
...
1. Create `features/support/components/SupportInput.tsx`:
   ...
   - Primary CTA button "Start Analysis in Bot" that deep-links to bot using openTelegramLink:
     ...
     ```
   - **IMPORTANT (SUPP-01 screenshot affordance):** Add a dedicated secondary "Send Screenshot" button below the primary CTA. This button:
     - Uses Camera icon from lucide-react
     - Label: "Send Screenshot for Analysis"
     - Subtitle text: "Send a screenshot of a conversation, profile, or post for AI visual analysis"
     - Deep-links to bot with a photo-specific start param: `https://t.me/${botUsername}?start=support_photo`
     - Uses openTelegramLink (same pattern as primary CTA)
     - Styled as a secondary/outline variant button, visually distinct from the primary CTA but clearly actionable
     - This is NOT just descriptive text -- it is a dedicated interactive button that users can tap
   - Use openTelegramLink from '@telegram-apps/sdk-react' (not openLink, per Phase 3 decision)
   - Style: Use Card component, Button with variant="primary" for text analysis, Button with variant="secondary" or outline style for screenshot. Use lucide-react icons (MessageSquare, ArrowRight, Camera)
...
7. Replace `pages/Support.tsx` with nested sub-routes:
  ...
  <verify>
Run `cd /Users/dmytrolevin/Downloads/GD_playground && pnpm --filter webapp exec tsc --noEmit` to confirm TypeScript compilation passes. Then run `cd /Users/dmytrolevin/Downloads/GD_playground && pnpm --filter webapp build` to confirm production build succeeds. Verify SupportInput.tsx contains both a primary "Start Analysis" button AND a secondary "Send Screenshot" button with Camera icon, each with its own openTelegramLink call.
  </verify>
  <done>
Support page renders with three sub-routes. Index shows input CTA with BOTH "Start Analysis" primary button AND "Send Screenshot for Analysis" secondary button (with Camera icon). Session detail shows full analysis with all four sections (analysis, strategy, tactics, draft). Copy button uses clipboard API with fallback. Regenerate and save-to-casebook buttons deep-link to bot. Session cards show prospect info extracted from output_json. Build passes cleanly.
  </done>
</task>
...
9. SupportInput.tsx has a dedicated "Send Screenshot" button (not just text mention) with Camera icon and openTelegramLink deep link to bot
</verification>

<success_criteria>
- All 5 SUPP requirements addressed: SUPP-01 (input CTA with explicit screenshot upload button), SUPP-02 (analysis display), SUPP-03 (engagement tactics), SUPP-04 (draft with copy/regenerate/save), SUPP-05 (session history)
- SUPP-01 screenshot option is a dedicated interactive button (Camera icon, "Send Screenshot for Analysis"), not just descriptive text
