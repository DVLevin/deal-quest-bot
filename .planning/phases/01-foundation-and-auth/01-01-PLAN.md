...
must_haves:
  truths:
    ...
    - "TypeScript interfaces in shared/tables.ts match the actual InsForge database schema columns (validated via mcp__insforge__get-table-schema)"
  artifacts:
...
<task type="auto">
  ...
  <verify>
    ...
    5. **Schema validation against live DB**: Use `mcp__insforge__get-table-schema` for each of the key tables (users, attempts, track_progress, lead_registry) and compare the returned columns against the TypeScript interfaces in tables.ts. Fix any mismatches. Columns in InsForge but not in the TS interface must be added. Columns in the TS interface but not in InsForge should be marked with a `// NOTE: column may not exist yet` comment.
  </verify>
  <done>
