...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    10. **RLS data isolation verification**: Create a second test user with a different Telegram account (or use a second Telegram account). Open the TMA as User A -- note the telegram_id displayed on the Dashboard. Open the TMA as User B (different Telegram account) -- verify User B sees their OWN telegram_id, NOT User A's. If both users see the same data or each other's data, RLS policies are not working correctly and must be debugged before proceeding.
    11. **API authentication verification**: Open browser dev tools (Network tab), trigger any InsForge query (e.g., navigate to a page that fetches data, or just observe the auth flow). Verify:
        - At least one request to the InsForge URL includes an `Authorization: Bearer <jwt>` header (not just the anon key)
        - The response contains user-scoped data (not all rows from the table)
        - If using the Edge Function proxy fallback (Task 2b was executed), verify requests go to `data-proxy` with `X-Telegram-JWT` header instead
    ...
    - RLS isolation testing requires two real Telegram accounts opening the TMA separately
  </how-to-verify>
