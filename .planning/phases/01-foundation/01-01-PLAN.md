...
must_haves:
  truths:
    ...
    - "ContextVar trace_id propagates across async boundaries (asyncio.gather, create_task)"
    - "Spans can be recorded via traced_span decorator with timing, parent linkage, and I/O capture"
    - "TraceCollector buffers traces/spans and flushes to InsForge via TraceRepo"
...
Purpose: Create the foundational TraceContext context manager (mirroring ProgressUpdater pattern), ContextVar-based trace propagation, span recording decorator, batched collector, and InsForge storage — everything needed before instrumenting handlers.
...
<task type="auto">
  <name>Task 1: Create tracing module with TraceContext, traced_span, and TraceCollector</name>
  <files>
    bot/tracing/__init__.py
    bot/tracing/context.py
    bot/tracing/collector.py
  ...
  <action>
Create `bot/tracing/` package with four files:
...
```
  ...
  <done>
    - TraceContext can be instantiated and used as async context manager
    - traced_span decorator wraps async functions and records spans when trace_id is set
    - TraceCollector buffers and flushes via background task
    - All imports resolve without errors
  </done>
</task>

<task type="auto">
  <name>Task 2: Create TraceRepo and add Pydantic storage models</name>
  <files>
    bot/storage/repositories.py
    bot/storage/models.py
  </files>
...
<task type="auto">
  <name>Task 3: Create InsForge SQL migration for traces and spans tables</name>
...
Verify TraceContext can be used as async context manager:
```bash
python -c "
import asyncio
...
async def test():
    async with TraceContext('test', 123, 1) as t:
        assert _trace_id.get() == t.trace_id
        print(f'trace_id inside: {t.trace_id}')
