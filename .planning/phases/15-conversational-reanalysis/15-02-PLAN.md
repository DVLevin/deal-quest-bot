...
Output: New context_input handler module, updated FSM states, reminder context callback, wired to main.py.
</objective>
...
async def on_context_add_start(
    ...
    await callback.message.edit_text(  # type: ignore[union-attr]
        ...
        "• ↪️ *Forward* — Forward a prospect's message\n"
...
reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    [
        InlineKeyboardButton(text="📝 Add Context", callback_data=f"context:add:{lead_id}"),
    ],
    [
        InlineKeyboardButton(text="📋 View Lead", callback_data=f"lead:view:{lead_id}"),
    ],
...
<task type="auto">
  <name>Task 4: Wire context_input router to main.py</name>
...
Add to router includes (after reminders_router):
```python
dp.include_router(context_input_router)
```

Also ensure the assemblyai_api_key is available for voice transcription:
- It should already be passed via DI from config (check config.py for ASSEMBLYAI_API_KEY)
- If not, add it to the dependency injection similar to other API keys
  </action>
  <verify>grep for "context_input_router" in bot/main.py</verify>
  <done>Context input router is wired to the dispatcher</done>
</task>
...
4. Add Context button in reminders: `grep "context:add" bot/handlers/reminders.py`
5. All input types handled: `grep -E "(on_context_text|on_context_voice|on_context_photo|on_context_forward)" bot/handlers/context_input.py`
</verification>
...
- "Re-analyze Strategy?" button appears after context is added
- Reminder messages include "Add Context" button
