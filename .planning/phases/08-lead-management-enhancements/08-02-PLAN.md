...
<task type="auto">
  <name>Task 2: Update Leads page to accommodate enhanced list</name>
  <files>
    packages/webapp/src/pages/Leads.tsx
  </files>
  <action>
    The current Leads.tsx page has minimal structure. With the enhanced LeadList now containing search/filter
    controls, the page layout needs minor adjustments to ensure proper spacing.

    MODIFY `packages/webapp/src/pages/Leads.tsx`:
    - The page currently wraps content in `<div className="space-y-4 px-4 pt-4 pb-6">`. This should
      remain as-is since LeadList's internal spacing handles the search/filter/list gaps.
    - No structural changes needed to the page itself -- LeadList is self-contained.
    - However, verify the page title + LeadList spacing works visually. If needed, adjust the
      `space-y-4` to `space-y-3` for tighter mobile spacing between the "Leads" heading and the search bar.

    This task is intentionally minimal -- the LeadList component handles all the complexity.
    The only change is to verify Leads.tsx renders correctly with the enhanced LeadList and
    make any minor spacing adjustments if needed.

    If no changes are needed after reviewing the page renders correctly with enhanced LeadList,
    document this in the commit as "verified, no changes needed."
  </action>
  <verify>
    Run `cd /Users/dmytrolevin/Downloads/GD_playground/packages/webapp && npx tsc --noEmit` to verify no TypeScript errors.
    Run `cd /Users/dmytrolevin/Downloads/GD_playground/packages/webapp && npx vite build` to verify the full build succeeds.
  </verify>
  <done>
    - Leads page renders enhanced LeadList with search, filter, and grouping
    - Full Vite build succeeds without errors
    - Page spacing is appropriate for mobile Telegram Mini App
  </done>
</task>
