...
4. **Update handler PipelineContext construction:**
   ...
   BUT -- modifying handlers is in Plan 02's scope. For this plan, just make model_config an optional parameter (default None) in PipelineContext.__init__ so it doesn't break existing handler code. Plan 02 handlers already create wrapper functions; they can pass model_config_service through.

   Actually, looking more carefully: Plan 02 modifies handler files to replace TraceContext. The model_config_service injection into PipelineContext should be done there too. For THIS plan, ensure PipelineContext accepts model_config=None gracefully.

   WAIT -- Plan 02 depends on Plan 01, not Plan 03. Plans 01 and 03 are parallel (Wave 1). Plans 02 and 04 are Wave 2. Plan 02 touches the same handler files. To avoid conflicts:

   Add a simple note in the PipelineContext: model_config defaults to None. The handlers ALREADY create PipelineContext -- they'll need updating. Since Plan 02 rewrites the handler pipeline sections anyway, include a note in Plan 02 that they should pass model_config_service from workflow_data into PipelineContext.

   For THIS plan: just ensure PipelineContext.__init__ has `model_config=None` as optional param, and get_llm_for_agent handles None gracefully.
  </action>
  <verify>
