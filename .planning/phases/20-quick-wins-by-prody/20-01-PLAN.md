...
must_haves:
  truths:
    ...
    - "XP award does not double-fire when status is changed from both TMA and bot — guarded by lead_activity_log xp_award check"
  artifacts:
    - path: "packages/webapp/src/features/leads/hooks/useUpdateLeadStatus.ts"
      provides: "XP award + level recalculation on closed_won status change, with double-fire guard"
      contains: "closed_won"
    ...
    - path: "bot/handlers/leads.py"
      provides: "Bot-side XP award on closed_won status change, with double-fire guard"
      contains: "update_xp"
  ...
  key_links:
    ...
    - from: "packages/webapp/src/features/leads/hooks/useUpdateLeadStatus.ts"
      ...
      via: "Check for existing xp_award before awarding, insert xp_award after awarding"
      pattern: "xp_award"
...
<task type="auto">
  <name>Task 1: TMA deal closure celebration + XP award with double-fire guard</name>
  <files>
    packages/webapp/src/features/leads/hooks/useUpdateLeadStatus.ts
    packages/webapp/src/features/leads/components/LeadDetail.tsx
  </files>
...
1. After the successful activity log insert in `mutationFn`, add a `closed_won` check with a double-fire guard:
   ```
...
Grep confirms `xp_award` guard check in useUpdateLeadStatus.ts (the double-fire prevention).
Grep confirms `fireLevelUpConfetti` import in LeadDetail.tsx.
  </verify>
  <done>
Changing a lead to closed_won in the TMA awards 500 XP (only if not already awarded for this lead), recalculates level, triggers confetti, and shows a "Deal Won! +500 XP" toast. An xp_award activity_log record is inserted as a guard marker. User progress cache is invalidated so ProgressCard updates immediately.
  </done>
</task>

<task type="auto">
  <name>Task 2: Bot-side XP award with double-fire guard + "Start Free" onboarding label</name>
  <files>
    bot/handlers/leads.py
    bot/handlers/start.py
  </files>
...
1. Add `UserRepo` and `LeadActivityRepo` to the `on_lead_status_update` function signature (DI injection): `async def on_lead_status_update(callback: CallbackQuery, lead_repo: LeadRegistryRepo, user_repo: UserRepo, activity_repo: LeadActivityRepo) -> None:`
2. After `await lead_repo.update_status(lead_id, new_status)` (line ~449), add:
   ...
   if new_status == "closed_won" and callback.from_user:
       ...
       existing = await activity_repo.client.query(
           "lead_activity_log",
           params={
               "lead_id": f"eq.{lead_id}",
               "activity_type": "eq.xp_award",
               "limit": 1,
           },
       )
       if not existing:
           ...
           await activity_repo.client.insert(
               "lead_activity_log",
               {
                   "lead_id": lead_id,
           ...
           await callback.answer("Deal Won! (XP already awarded)")
   else:
...
3. Add `UserRepo` and `LeadActivityRepo` imports if not already imported at the top of the file.
...
Note: The bot uses aiogram DI -- adding `user_repo: UserRepo` and `activity_repo: LeadActivityRepo` to the handler signature is sufficient for injection since they're already in `workflow_data` (wired in main.py).
  </action>
  <verify>
...
Grep confirms "Use My Own Key" in start.py.
Grep confirms "update_xp" call in leads.py on_lead_status_update.
Grep confirms "xp_award" guard check in leads.py (the double-fire prevention).
  </verify>
  <done>
Bot awards 500 XP when a lead's status is changed to closed_won via the bot, but only if XP was not already awarded for this lead (double-fire guard via lead_activity_log xp_award check). Onboarding button reads "Start Free (Recommended)" instead of "Quick Setup (Recommended)", and "Use My Own Key" instead of "Use My Own API Key".
  </done>
</task>
...
<verification>
1. `useUpdateLeadStatus.ts` contains a `closed_won` branch that checks for existing xp_award, awards 500 XP only if none exists, recalculates level, and inserts an xp_award guard marker
2. `LeadDetail.tsx` imports `fireLevelUpConfetti` and fires it on closed_won status change
3. `leads.py` `on_lead_status_update` checks lead_activity_log for existing xp_award before calling `user_repo.update_xp(tg_id, 500)`, then inserts xp_award marker
4. `start.py` `SETUP_METHOD_KEYBOARD` contains "Start Free" text and "Use My Own Key" text
5. User progress query is invalidated after XP award (both TMA and bot paths)
6. Double-fire prevention: if user closes deal from TMA then bot (or vice versa), XP is only awarded once
</verification>

<success_criteria>
- Deal closure (closed_won) awards 500 XP from both TMA and bot surfaces (whichever fires first)
- XP is NOT double-awarded if both TMA and bot process the same lead closure
...
- Second onboarding button says "Use My Own Key" (not "Use My Own API Key")
</success_criteria>
