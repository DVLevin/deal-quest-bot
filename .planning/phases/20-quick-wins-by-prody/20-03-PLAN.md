...
must_haves:
  ...
  key_links:
    ...
    - from: "packages/webapp/src/features/leads/hooks/useUpdatePlanStep.ts"
      ...
      via: "onSuccess callback triggers status suggestion check using fresh cache data"
...
<task type="auto">
  ...
  <action>
Add a smart status suggestion that appears as a toast after completing engagement plan steps. IMPORTANT: Do NOT build a simulated plan from stale data. Instead, perform the suggestion check after query invalidation settles so we use fresh cache data.

1. Import `suggestNextStatus` and `LEAD_STATUS_CONFIG` from `../types` (if not already imported).
2. Import `useQueryClient` from `@tanstack/react-query` (if not already imported).
3. Create a helper function (inside the component or as a module-level utility) that checks if a status suggestion is warranted based on step completion progress:
   ```typescript
   ...
   ```
4. In the step update mutation's `onSuccess` callback (NOT in the optimistic update or mutationFn), after query invalidation has settled, read the FRESH lead data from the query cache to perform the suggestion check. The approach:
   ```typescript
   // Inside the onSuccess callback of useUpdatePlanStep (or wherever step completion is handled in LeadDetail):
   onSuccess: async (_data, variables) => {
     // Wait for query invalidation to settle so cache has fresh data
     await queryClient.invalidateQueries({ queryKey: queryKeys.leads.detail(leadId) });

     // Only check suggestion when step was marked as done
     if (variables.newStepStatus === 'done') {
       // Read fresh lead from cache after invalidation
       const freshLead = queryClient.getQueryData<Lead>(queryKeys.leads.detail(leadId));
       if (freshLead) {
         const suggestedStatus = shouldSuggestStatusChange(freshLead.engagement_plan, freshLead.status);
         if (suggestedStatus) {
   ...
   ```
   This approach avoids the fragile pattern of manually simulating plan state from potentially stale `lead` data. By reading from the query cache AFTER invalidation, we get the server-confirmed state.
5. The toast includes an "Update" action that directly triggers `handleStatusChange` -- this is the existing function in LeadDetail that fires the status mutation. The user can tap "Update" to progress, or dismiss the toast.
6. Make sure the suggestion only fires when `newStepStatus === 'done'` (not on skip or reset) to avoid noisy suggestions.
7. Import `EngagementPlanStep` type from `@/types/tables` if not already imported.
...
Grep confirms `getQueryData` usage (reading fresh data from cache, not simulating).
  </verify>
  <done>
After marking an engagement step as Done, the query cache is invalidated and fresh lead data is read. If 50%+ of steps are completed, a toast appears: "Great progress! Move to 'Reached Out'?" with an "Update" action button that triggers the status change. Only fires on 'done' steps (not skip/reset). Uses fresh server data, not stale optimistic simulation.
  </done>
</task>
...
4. Toast suggestion reads fresh data from query cache after invalidation (not simulated from stale state)
5. Toast includes an actionable "Update" button that triggers the existing status change flow
...
- Step completion triggers smart status suggestion toast at 50%+ completion using fresh cache data
